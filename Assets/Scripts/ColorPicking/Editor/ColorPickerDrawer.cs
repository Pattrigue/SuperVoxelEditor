...
namespace SemagGames.VoxelEditor.ColorPicking.Editor
{
    ...
    public sealed class ColorPickerDrawer : PropertyDrawer
    {
        ...
        private const string SelectedColorIndex = "selectedColorIndex";
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            SerializedProperty selectedColorIndex = property.FindPropertyRelative(SelectedColorIndex);
            ...
            if (colorPaletteTextureProp.objectReferenceValue != null)
            {
                ...
                // Set the colors array in the ColorPicker class
                ...
                for (int i = 0; i < colors.Length; i++)
                {
                    ...
                    if (colors[i] == selectedColorProp.colorValue)
                    {
                        EditorGUI.DrawRect(new Rect(colorRect.x - 1, colorRect.y - 1, cellSize + 2, cellSize + 2), Color.white);
                    }
                    ...
                    if (GUI.Button(colorRect, GUIContent.none, emptyStyle))
                    {
                        ...
                        selectedColorIndex.intValue = i;
                    }
                }

                position.y += Mathf.CeilToInt((float)colors.Length / colorsPerRow) * cellWidth;
             
                // Draw the "Selected color index" label
                EditorGUI.LabelField(new Rect(position.x, position.y, EditorGUIUtility.labelWidth, EditorGUIUtility.singleLineHeight), $"Selected color index: {selectedColorIndex.intValue}", EditorStyles.boldLabel);
            }
